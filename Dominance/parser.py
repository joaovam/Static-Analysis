...
def create_Bt(curr, insts):
    ...
    inst = Bt(b_t)
    inst.ID = index
    if type(insts[int(ops[2])]) != tuple:
        
        inst.add_true_next(insts[int(ops[2])])
    else:
        next = create_Bt(insts[int(ops[2])], insts)
        insts[next.ID] = next
        inst.add_true_next(next)
    return inst
...
def file2cfg_and_env(lines):
    ...
    for l in lines:
        ...
        if ops[0] == "bt":
            ...
            insts.append((l, Inst.next_index))
            Inst.next_index += 1
    ...
    for i in range(len(insts)):
        curr = insts[i]
        #print("CURR0", curr)
        if type(curr) == tuple: #it is a BT waiting for connection
            #print("CURR1", curr)
